...
router.post("/questions", async (req, res, next) => {
  ...
  try {
    await pool.query(
      `INSERT INTO questions (title, email, value) VALUES (?, ?, ?)`,
      [title, email, value]
    );
    res.status(204).json();
  } catch (err) {
  ...
  try {
    if (sort === "answered") {
      const [questions] = await pool.query(
        `SELECT id, title, email, value, reply FROM questions WHERE reply IS NOT NULL`
      );
      res.status(200).json({ questions });
    } else {
      const [questions] = await pool.query(
        `SELECT id, title, email, value FROM questions WHERE reply IS NULL`
      );
      res.status(200).json({ questions });
    }
  } catch (err) {
...
router.post("/questions/:question_id/reply", async (req, res, next) => {
  ...
  try {
    ...
    const [
      rows
    ] = await pool.query(`UPDATE questions SET reply = ? WHERE id = ?`, [
      reply,
      question_id
    ]);
...
router.get("/notices/:notice_id", async (req, res, next) => {
  ...
  try {
    const [
      ...
      `SELECT id, title, value, created_at FROM notices WHERE id = ?`,
      [notice_id]
    );
...
router.post("/notices", async (req, res, next) => {
  ...
  try {
    await cert(req);
    await pool.query(`INSERT INTO notices (title, value) VALUES (?, ?)`, [
      title,
      value
    ]);
    res.status(204).json();
...
router.post("/push", async (req, res, next) => {
  ...
  const getRegId = async (minAge, maxAge, gender) => {
    ...
    const [
      ...
      `SELECT reg_id FROM users WHERE DATE(birthday) BETWEEN ? AND ? AND gender=?`,
      [minBirthday, maxBirthday, gender]
    );
